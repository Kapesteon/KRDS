...
from gestion import HOME, HOST, PORT, SESSION_USER, SLASH, exitError
from hashlib import sha256
from os import chdir, mkdir, remove, walk
...
def getSessionUser():
    ...
    if not exists(SESSION_USER):
        return exitError(f'{SESSION_USER} doesn\'t exist')

    with open(SESSION_USER,'r') as sessionFile:
        userID = sessionFile.read()

        if not userID.isdigit():
            exitError('user_id no retrieved, the data of the session has been lost.')

        print('[INFO] : User id successfuly retrieved')
...
def cleanUp(home):
    if exists(SESSION_USER): remove(SESSION_USER)
    chdir(f'..{SLASH}')
    if exists(home): rmtree(home)
    mkdir(home)
